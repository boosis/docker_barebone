...
RUN no ""|pecl install mongo-alpha && echo "extension=mongo.so" > /etc/php5/mods-available/mongo.ini && cd /etc/php5/cli/conf.d && ln -s ../../mods-available/mongo.ini 20-mongo.ini && cd /etc/php5/fpm/conf.d && ln -s ../../mods-available/mongo.ini 20-mongo.ini
